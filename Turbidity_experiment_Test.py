...
def sd_func(d):
    return np.sqrt(np.sum(np.square(d - np.mean(d)))/(len(d)-1)) #standard deviation function
...
popt, pcov = curve_fit(exponential_func, x, y, p0=(1, 1e-6, 1)) #creates the exponential fit function for x,y and with the parameters
 
xx = np.linspace(0, 20) #set xx as linearly spaced over your time interval
